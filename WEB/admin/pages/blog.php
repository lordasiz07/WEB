...
<?php
//Aktifliğini değiştirme kodları
@$id= intval(@$_GET["id"]);
if(@$_GET["is"]=="aktif") {
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    <?php
                                    $cek = $db->prepare("SELECT * FROM `blog` ORDER BY `id` DESC"); //VERİTABANINDAN GELENLERİN DÖNGÜ İLE YAZDIRILMASI.
                                    $cek->execute();
                                    ...
                                    ?>    
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <?php if($row["aktif"]==1) { ?>
                                                ...
                                                onclick="return confirm ('Aktiflik Durumu Değişsin Mi?')" class="btn
                                                btn-success btn-xs" style ="margin-right: 15px;">Aktif</a>
                                                <?php } else { ?>
                                                    <a href="blog.php?is=aktif&id=<?= $row["id"] ?>&drm=<?= $row["aktif"] ?>"
                                                onclick="return confirm ('Aktiflik Durumu Değişsin Mi?')" class="btn
                                                btn-danger btn-xs" style ="margin-right: 15px;">Pasif</a>
                                                <?php } ?>
                                                    
                                            </td>
                                            <td class="center">
                                            <a href="blog_duzenle.php?id=<?= $row["id"] ?>" class="btn btn-warning
                                             btn-xs" style="margin-right: 15px;">Düzenle</a>
                                            <a href="blog.php?is=sil&id=<?= $row["id"] ?>" onclick="return confirm
                                             ('Silmek istediğinize eminmisiniz?')" class="btn btn-danger btn-xs" style="margin-right: 15px;">Sil</a>
                                            </td>
                                        </tr>
                                    <?php } ?>
